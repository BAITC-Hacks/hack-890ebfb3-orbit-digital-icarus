...
def assert_eligible(request: MatchRequestLike, contractor: ContractorLike) -> None:
    ...
    checks = (
        ...
        request.category in contractor.categories,
        iso_date(request.event_date) not in {iso_date(day) for day in contractor.busy_dates},
        contractor.price_from_kzt <= request.budget_kzt,
        request.event_format in contractor.event_formats,
        request.language is None or request.language in contractor.languages,
        request.duration_hours is None or contractor.max_hours is None
        or request.duration_hours <= contractor.max_hours,
    )
    if not all(checks):
...
def rank_candidates(
    ...
    eligible: Iterable[ContractorLike],
    ...
    for contractor in eligible:
        ...
        records = evidence.get(contractor.id, ())
        relevant = sorted(
            (item for item in records if request.event_format in item.event_formats),
